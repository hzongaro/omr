      ...
      if (divisor != 0)
         ...
         if (firstChild->getOpCode().isLoadConst())
            ...
            if (node->getOpCode().isUnsigned())
               ...
               if (foldUIntConstant(node, uquotient, s, false /* !anchorChildren*/))
                  {
                  // If folding to a constant actually happened, any parent DIVCHK is no longer needed
            ...
            {
            TR::Node *replacementNode = s->replaceNode(node, firstChild, s->_curTree, block);
            ...
            if (performTransformation(s->comp(), "%sReduced idiv by -1 with ineg in node [%s]\n", s->optDetailString(), node->getName(s->getDebug())))
               ...
               // Division has been removed, so any parent DIVCHK is no longer needed
               s->_nodeToDivchk = NULL;
               }
            }
         // Design 1790
      ...
      if (divisor != 0)
         ...
         if (firstChild->getOpCode().isLoadConst())
            ...
            if (divisor == -1 && dividend == TR::getMinSigned<TR::Int64>())
               ...
               // Verify that replaceNode has actually replaced the ldiv with its first
               // child - replacement is guarded by performTransformation.  If replacement
               // happened, any parent DIVCHK is no longer needed.
               //
               if (replacementNode == firstChild)
            ...
            if (foldLongIntConstant(node, dividend / divisor, s, false /* !anchorChildren */))
               {
               // If folding to a constant actually happened, any parent DIVCHK is no longer needed
               s->_nodeToDivchk = NULL;
               }
...
TR::Node *bdivSimplifier(TR::Node * node, TR::Block * block, TR::Simplifier * s)
   ...
   if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst())
      {
      if (foldByteConstant(node, (int8_t)(firstChild->getByte() / secondChild->getByte()), s, false /* !anchorChildren*/))
         {
         // If folding to a constant actually happened, any parent DIVCHK is no longer needed
         s->_nodeToDivchk = NULL;
         }
...
TR::Node *sdivSimplifier(TR::Node * node, TR::Block * block, TR::Simplifier * s)
   ...
   if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst())
      {
      if (foldShortIntConstant(node, (int16_t)(firstChild->getShortInt() / secondChild->getShortInt()), s, false /* !anchorChildren */))
         {
         // If folding to a constant actually happened, any parent DIVCHK is no longer needed
         s->_nodeToDivchk = NULL;
         }
   ...
   if (secondChild->getOpCode().isLoadConst())
      ...
      if (divisor != 0)
         ...
         if (divisor == 1 || (!isUnsigned && (divisor == -1)))
            {
            if (foldIntConstant(node, 0, s, true /* anchorChildren */))
               {
               // If folding to a constant actually happened, any parent DIVCHK is no longer needed
               s->_nodeToDivchk = NULL;
               }
            ...
            if (foldIntConstant(node, result, s, false /* !anchorChildren */))
               {
               // If folding to a constant actually happened, any parent DIVCHK is no longer needed
               s->_nodeToDivchk = NULL;
               }
   ...
   if (secondChild->getOpCode().isLoadConst())
      ...
      if (divisor != 0)
         ...
         if (divisor == 1 || (divisor == -1))
            {
            if (foldLongIntConstant(node, 0, s, true /* anchorChildren */))
               {
               // If folding to a constant actually happened, any parent DIVCHK is no longer needed
               s->_nodeToDivchk = NULL;
               }
            ...
            if (divisor == -1 && dividend == TR::getMinSigned<TR::Int64>())
               folded = foldLongIntConstant(node, 0, s, false /* !anchorChildren */);
            else
               folded = foldLongIntConstant(node, dividend % divisor, s, false /* !anchorChildren */);
...
TR::Node *divchkSimplifier(TR::Node * node, TR::Block * block, TR::Simplifier * s)
   ...
   if (child->getVisitCount() != s->comp()->getVisitCount())
      ...
      if (childOpCode.isDiv() || childOpCode.isRem())
         ...
         // is used node as the child of the DIVCHK; otherwise, the DIVCHK is
         // simplified to a treetop node.
         //
         ...
         // DIVCHK would be n197n, which is itself an idev, while for the trees on
      ...
      // DIVCHK is no longer needed.
      //
      if (performTransformation(s->comp(), "%sRemoved DIVCHK for commoned division operation in node[%s]\n", s->optDetailString(), node->getName(s->getDebug())))
         {
         TR::Node::recreate(node, TR::treetop);
         return node;
         }
